...
The layout is a simple vertical mission path rather than an animated scene. Completed Genesis is visually subdued with a check marker. `Mid-Season 1 Update` is the brightest milestone because it is the next planned release. Later milestones progressively recede visually. Season 2 includes a simple static orbital motif. Season 3 includes a faint static starship silhouette.
...
Season 1 assets carry forward into Season 2 and play a crucial role in the new Stellar Expansion mechanics. This note is visually separated behind a small `Legacy` info control with hover/focus/tap disclosure.
...
Keep motion intentionally restrained. Adapt the pointer spotlight interaction from React Bits `SpotlightCard` for the active Mid-Season milestone only. No parallax, scroll choreography, continuous motion, or 3D. Respect keyboard focus and reduced-motion expectations.
...
## Verification
Add route coverage and page rendering/accessibility coverage. The repository pull-request verification workflow must pass database validation, lint, typecheck, tests, and build.
